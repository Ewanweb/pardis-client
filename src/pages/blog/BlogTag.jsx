...
const BlogTag = () => {
  ...
  useEffect(() => {
    ...
    const loadTagInfo = async () => {
      try {
        ...
        if (!isMounted) return;
        const current = tags.find((item) => item.slug === slug);
        setTagInfo(current || null);
      } catch (err) {
        if (!isMounted) return;
        setTagInfo(null);
      }
  ...
  useEffect(() => {
    ...
    const loadPosts = async () => {
      ...
      setError(null);
      try {
        const result = await blogService.getPostsByTag(slug, {
          page,
          pageSize: PAGE_SIZE
        });
        if (!isMounted) return;
  ...
  const seoConfig = generateSEOConfig({
    fallbackTitle: tagInfo?.title ? `مقالات با برچسب ${tagInfo.title}` : 'برچسب‌های وبلاگ',
    fallbackDescription: 'مقالات آموزشی بر اساس برچسب‌های منتخب در آکادمی پردیس توس',
    currentUrl: `/blog/tag/${slug}`
  });
  ...
  const breadcrumbsSchema = {
    ...
    ...buildBreadcrumbSchema([
      ...
      { name: 'وبلاگ', url: '/blog' },
      { name: tagInfo?.title || 'برچسب', url: `/blog/tag/${slug}` }
    ])
  };
  ...
  return (
    <div className="min-h-screen pt-28 pb-20 bg-slate-50 dark:bg-slate-950 transition-colors duration-300">
      ...
      />
      <Helmet>
        {meta.page > 1 && <link rel="prev" href={`/blog/tag/${slug}?page=${meta.page - 1}`} />}
        {meta.page < meta.totalPages && (
          <link rel="next" href={`/blog/tag/${slug}?page=${meta.page + 1}`} />
        )}
      </Helmet>
      ...
      <div className="container mx-auto px-4">
        <section className="relative overflow-hidden rounded-[2.5rem] bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 p-8 md:p-12 shadow-xl shadow-slate-200/40">
          <div className="absolute -top-20 -left-20 w-72 h-72 bg-secondary-500/10 rounded-full blur-3xl" />
          <div className="absolute -bottom-20 -right-20 w-72 h-72 bg-primary-500/10 rounded-full blur-3xl" />
          <div className="relative z-10 flex flex-col md:flex-row md:items-center gap-6">
            <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-secondary-500 to-primary-600 flex items-center justify-center text-white shadow-lg">
              <Hash size={30} />
            </div>
            <div>
              <h1 className="text-3xl md:text-4xl font-black text-slate-900 dark:text-white">
                {tagInfo?.title ? `برچسب #${tagInfo.title}` : 'برچسب‌های وبلاگ'}
              </h1>
              <p className="text-slate-500 dark:text-slate-400 mt-3 max-w-2xl">
                مقاله‌هایی که با این برچسب مرتبط هستند را مرور کنید و به عمق موضوع برسید.
              </p>
            </div>
          </div>
        </section>
        ...
        <div className="mt-10">
          {loading ? (
            ...
            <div className="bg-white dark:bg-slate-900 rounded-3xl border border-red-100 dark:border-red-900/40 p-8 text-center">
              <p className="text-red-600 dark:text-red-400 font-bold">دریافت مقالات این برچسب با خطا مواجه شد.</p>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-3xl border border-dashed border-slate-300 dark:border-slate-700 p-12 text-center">
              <p className="text-slate-500 dark:text-slate-400">هنوز مقاله‌ای با این برچسب منتشر نشده است.</p>
