...
import { Sparkles, LogOut, BookOpen, Search, Globe, Share2, Eye, EyeOff, AlertCircle, ChevronDown, Edit, Save, UploadCloud, Loader2, X, CheckCircle2, ChevronLeft, DollarSign, FileText, Image as ImageIcon, Trash2, RefreshCcw, Ban, AlertTriangle, User, Calendar, Clock, PlayCircle, List, Plus } from 'lucide-react';
...
const STATUS_LABELS = {
    ...
    'Archived': 'آرشیو شده', 'archived': 'آرشیو شده'
};

// ✅ دیکشنری رنگ‌ها
const STATUS_COLORS = {
    'Draft': 'amber', 'draft': 'amber',
    'Published': 'emerald', 'published': 'emerald',
    'Archived': 'slate', 'archived': 'slate'
};

// ✅ تابع ترجمه خطاهای بک‌اند
...
const AdminCourses = () => {
    ...
    const initialFormState = {
        ...
        start_from: '', schedule: '', is_completed: false, is_started: false,
        sections: [], // آرایه سرفصل‌ها
    ...
    useEffect(() => { fetchCourses(); }, [showTrashed, isMyCourses]);

    useEffect(() => {
        fetchCategories();
        if (!isMyCourses && hasRole(['Admin', 'Manager'])) {
            fetchInstructors();
        }
    }, [isMyCourses]);
    ...
    const handleEditClick = (course) => {
        ...
        const catId = course.category?.id || course.categoryId || course.category_id || '';
        const instId = course.instructor?.id || course.instructorId || course.instructor_id || '';

        // ✅ دریافت تاریخ دقیقاً همانطور که از سرور می‌آید (شمسی)
        const dateStr = course.startFrom || course.StartFrom || '';
        ...
        setFormData({
            ...
            // ✅ تبدیل مقادیر بولین دریافتی به بولین جاوااسکریپت (برای چک‌باکس‌ها/تایگل‌ها)
            is_completed: !!(course.isCompleted || course.IsCompleted),
            is_started: !!(course.isStarted || course.IsStarted),

            // ✅ پر کردن آرایه سرفصل‌ها از بک‌اند
            ...
            seo: {
                ...
                canonical_url: course.seo?.canonicalUrl || '',
    ...
    const nextStep = () => {
        ...
        if (currentStep === 2) {
            ...
            if (!isMyCourses && hasRole(['Admin', 'Manager']) && !formData.instructor_id) {
                return toast.error('لطفاً مدرس دوره را انتخاب کنید.');
            }
        }
    ...
    const handleSave = async () => {
        ...
        const savePromise = new Promise(async (resolve, reject) => {
            ...
            if (!isMyCourses && formData.instructor_id) {
                data.append('InstructorId', formData.instructor_id);
            }
            ...
            // برای ASP.NET Core نیازی به _method PUT نیست، اما اگر مشکل داشتید بگذارید بماند
    ...
    // ✅ اصلاح استپر: استفاده دقیق از currentStep
                                ...
                                {currentStep === 2 && (
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 animate-in fade-in slide-in-from-right-8 duration-300">
                                        ...
                                        <div className="space-y-6">
                                            ...
                                            <div><label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2 flex items-center gap-2"><Clock size={16}/> زمان‌بندی برگزاری</label><input type="text" className="w-full p-4 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none font-medium text-slate-800 dark:text-white transition-colors" name="schedule" value={formData.schedule} onChange={handleChange} placeholder="مثال: روزهای زوج ساعت ۱۸:۰۰" /></div>
                                            {!isMyCourses && hasRole(['Admin', 'Manager']) && (<div><label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2 flex items-center gap-2"><User size={16}/> مدرس دوره</label><div className="relative"><select className="w-full p-4 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 outline-none font-medium text-slate-700 dark:text-slate-200 appearance-none cursor-pointer transition-colors" name="instructor_id" value={formData.instructor_id} onChange={handleChange}><option value="" disabled>انتخاب مدرس...</option>{instructorsList?.map(inst => (<option key={inst.id} value={inst.id}>{inst.name || inst.fullName} ({inst.email})</option>))}</select><ChevronDown className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-500 pointer-events-none" size={20} /></div></div>)}
                                        </div>
                                    </div>
                                )}

                                {/* ✅ STEP 3: CURRICULUM (سرفصل‌ها) */}
                                {currentStep === 3 && (
                                                ...
                                                {formData.sections.map((section, index) => (
                                                    <div key={index} className="bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-200 dark:border-slate-700 overflow-hidden animate-in fade-in slide-in-from-top-2">
                                                        <div className="p-4 flex items-center gap-4 bg-white dark:bg-slate-900 border-b border-slate-100 dark:border-slate-800">
                                                            ...
                                                            {/* ✅ اصلاح value برای جلوگیری از ارور uncontrolled */}
                                                            <input
                                                                type="text"
                                ...
                                {currentStep === 6 && (
                                    <div className="flex flex-col items-center justify-center py-10 animate-in fade-in slide-in-from-right-8 duration-300">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 w-full max-w-3xl">
                                            ...
                                            {/* ✅ بخش وضعیت‌های جدید (IsStarted / IsCompleted) */}
                                            <div className="space-y-4">
                                                <label className="block text-sm font-bold text-slate-500 dark:text-slate-400 text-center">وضعیت برگزاری</label>
                                                <div className="flex flex-col gap-3">
                                                    <div
                                                        onClick={() => setFormData(prev => ({ ...prev, is_started: !prev.is_started }))}
                                                        className={`flex items-center gap-3 p-4 rounded-2xl border-2 transition-all cursor-pointer ${formData.is_started ? 'bg-indigo-50 dark:bg-indigo-900/20 border-indigo-500 text-indigo-600 dark:text-indigo-400' : 'bg-white dark:bg-slate-900 border-slate-100 dark:border-slate-800 text-slate-400'}`}
                                                    >
                                                        <div className={`w-10 h-10 rounded-full flex items-center justify-center ${formData.is_started ? 'bg-indigo-500 text-white' : 'bg-slate-100 dark:bg-slate-800'}`}>
                                                            <PlayCircle size={20} />
                                                        </div>
                                                        <div className="flex-1">
                                                            <p className="font-bold text-sm">دوره شروع شده است</p>
                                                            <p className="text-[10px] opacity-70">دانشجویان می‌توانند محتوا را ببینند</p>
                                                        </div>
                                                        <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${formData.is_started ? 'bg-indigo-500 border-indigo-500' : 'border-slate-300'}`}>
                                                            {formData.is_started && <CheckCircle2 size={14} className="text-white"/>}
                                                        </div>
                                                    </div>

                                                    <div
                                                        onClick={() => setFormData(prev => ({ ...prev, is_completed: !prev.is_completed }))}
                                                        className={`flex items-center gap-3 p-4 rounded-2xl border-2 transition-all cursor-pointer ${formData.is_completed ? 'bg-emerald-50 dark:bg-emerald-900/20 border-emerald-500 text-emerald-600 dark:text-emerald-400' : 'bg-white dark:bg-slate-900 border-slate-100 dark:border-slate-800 text-slate-400'}`}
                                                    >
                                                        <div className={`w-10 h-10 rounded-full flex items-center justify-center ${formData.is_completed ? 'bg-emerald-500 text-white' : 'bg-slate-100 dark:bg-slate-800'}`}>
                                                            <CheckCircle2 size={20} />
                                                        </div>
                                                        <div className="flex-1">
                                                            <p className="font-bold text-sm">دوره تکمیل شده است</p>
                                                            <p className="text-[10px] opacity-70">تمام جلسات بارگذاری شده‌اند</p>
                                                        </div>
                ...
                <table className="w-full text-right">
                    ...
                    {loading ? (<tr><td colSpan="6" className="text-center py-12 text-slate-400 dark:text-slate-500">در حال بارگذاری...</td></tr>) : courses?.length === 0 ? (<tr><td colSpan="6" className="text-center py-16"><p className="text-slate-400 dark:text-slate-500 text-sm font-bold">هیچ دوره‌ای یافت نشد!</p></td></tr>) : courses?.map(course => (
                        <tr key={course.id} className="group hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors">
                            ...
                            {/* ✅ نمایش نام مدرس (name یا fullName) */}
                            ...
                            <td className="px-6 py-4">
                                {/* ✅ استفاده از STATUS_LABELS و STATUS_COLORS */}
                                <Badge color={course.deleted_at ? 'red' : STATUS_COLORS[course.status] || 'slate'}>
                                    {course.deleted_at ? 'حذف شده' : STATUS_LABELS[course.status] || course.status}
                                </Badge>
                            </td>
