...
import { User, Mail, Phone, Lock, BookOpen, Award, Clock, Camera, Edit2, LogOut, Settings, LayoutDashboard, Shield, ChevronLeft, Calendar, CheckCircle2, TrendingUp, Zap, Activity, Bell, MapPin, Video, MonitorPlay } from 'lucide-react';
...
const UserProfile = () => {
    ...
    // ✅ کارت دوره جدید: مخصوص کلاس‌های زنده و حضوری
    const LiveCourseItem = ({ title, type, schedule, image, instructor, status, location }) => {
        const isOnline = type === 'online';
        const isCompleted = status === 'completed';

        return (
            <div className={`flex flex-col md:flex-row gap-6 p-5 bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 hover:border-primary/30 dark:hover:border-primary/30 hover:shadow-xl hover:shadow-primary/5 transition-all duration-500 group relative overflow-hidden ${isCompleted ? 'opacity-90 hover:opacity-100 grayscale hover:grayscale-0' : ''}`}>
                ...
                <div className="w-full md:w-56 h-48 md:h-auto min-h-[140px] rounded-[2rem] overflow-hidden shrink-0 relative shadow-md">
                    ...
                    <div className="absolute top-3 right-3">
                        <span className={`px-3 py-1.5 rounded-xl text-[10px] font-bold flex items-center gap-1.5 backdrop-blur-md shadow-lg border border-white/10 ${isCompleted ? 'bg-slate-600/90 text-white' : (isOnline ? 'bg-sky-500/90 text-white' : 'bg-rose-500/90 text-white')}`}>
                            {isCompleted ? <CheckCircle2 size={12} /> : (isOnline ? <Video size={12} /> : <MapPin size={12} />)}
                            {isCompleted ? 'پایان یافته' : (isOnline ? 'آنلاین (Live)' : 'حضوری')}
                        </span>
                    </div>
                ...
                <div className="flex-1 flex flex-col justify-center py-2 relative z-10">
                    <div className="flex justify-between items-start mb-3">
                        <div>
                            ...
                            <div className="flex items-center gap-4 text-xs font-medium text-slate-500 dark:text-slate-400">
                                ...
                                <span className="flex items-center gap-1 bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded-lg"><Clock size={12} className="text-amber-500"/> {status === 'active' ? 'در حال برگزاری' : 'تکمیل شده'}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="mt-auto bg-slate-50 dark:bg-slate-800/50 rounded-2xl p-4 border border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                        <div className="flex items-center gap-3">
                            <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${isCompleted ? 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600' : (isOnline ? 'bg-sky-100 dark:bg-sky-900/30 text-sky-600' : 'bg-rose-100 dark:bg-rose-900/30 text-rose-600')}`}>
                                {isCompleted ? <Award size={20}/> : (isOnline ? <MonitorPlay size={20}/> : <MapPin size={20}/>)}
                            </div>
                            <div>
                                <p className="text-[10px] font-bold text-slate-400 uppercase mb-0.5">
                                    {isCompleted ? 'وضعیت دوره' : (isOnline ? 'پلتفرم برگزاری' : 'محل برگزاری')}
                                </p>
                                <p className="text-sm font-bold text-slate-700 dark:text-slate-200 line-clamp-1">{location}</p>
                            </div>
                        </div>
                        ...
                        {isCompleted ? (
                            ...
                            <Button className={`w-full sm:w-auto !px-6 !py-2.5 !text-xs !rounded-xl shadow-none ${isOnline ? 'bg-sky-600 hover:bg-sky-700' : 'bg-slate-900 hover:bg-slate-800 dark:bg-white dark:text-slate-900'}`}>
                                {isOnline ? 'ورود به کلاس' : 'مسیر یابی'}
                            </Button>
                        )}
                    </div>
                </div>
                    ...
                    <div className="px-6 md:px-12">
                        <div className="flex flex-col md:flex-row items-end -mt-20 gap-8 relative z-20">
                            ...
                            <div className="flex gap-3 pb-6 w-full md:w-auto justify-center">
                                ...
                                <button
                                    onClick={() => setActiveTab('settings')}
                                    className="p-4 rounded-2xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-500 hover:text-primary transition-colors shadow-sm hover:shadow-md"
                                >
                                    <Settings size={24} />
                                </button>
                            </div>
                        ...
                        {activeTab === 'overview' && (
                            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-700">
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                                    <StatCard icon={BookOpen} label="کلاس‌های فعال" value="3" color="bg-indigo-500" bgClass="bg-indigo-500" trend="12"/>
                                    <StatCard icon={Clock} label="ساعات حضور" value="45h" color="bg-amber-500" bgClass="bg-amber-500" trend="5"/>
                                    <StatCard icon={Award} label="نمرات برتر" value="2" color="bg-emerald-500" bgClass="bg-emerald-500" />
                                </div>
                                ...
                                <div className="bg-white dark:bg-slate-900 p-1 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-sm">
                                    <div className="p-8 flex items-center justify-between">
                                        <div>
                                            <h3 className="text-2xl font-black text-slate-800 dark:text-white mb-1">برنامه کلاسی</h3>
                                            <p className="text-sm text-slate-500 dark:text-slate-400">کلاس‌های پیش‌رو و در حال برگزاری</p>
                                        </div>
                                        <Button variant="ghost" className="!text-xs" onClick={() => setActiveTab('courses')}>مشاهده همه</Button>
                                    </div>
                                    <div className="grid gap-6 px-6 pb-6">
                                        <LiveCourseItem
                                            title="بوت‌کمپ حضوری React و Next.js"
                                            instructor="مهندس حسینی"
                                            type="in-person"
                                            status="active"
                                            schedule="شنبه و دوشنبه، ۱۶:۰۰"
                                            location="تهران، دانشگاه شریف، کلاس ۱۰۲"
                                            image="https://placehold.co/600x400/1e1b4b/FFF?text=React+Bootcamp"
                                        />
                                        <LiveCourseItem
                                            title="دوره آنلاین متخصص پایتون"
                                            instructor="علی محمدی"
                                            type="online"
                                            status="active"
                                            schedule="چهارشنبه‌ها، ۱۸:۰۰"
                                            location="اسکای‌روم (لینک اختصاصی)"
                                            image="https://placehold.co/600x400/047857/FFF?text=Python"
                                        />
                                    </div>
                                </div>
                            </div>
                        ...
                        {activeTab === 'courses' && (
                            <div className="space-y-12 animate-in fade-in slide-in-from-bottom-8 duration-700">
                                ...
                                <div>
                                    ...
                                    <div className="grid gap-8 opacity-90 hover:opacity-100 transition-opacity">
                                        <LiveCourseItem
                                            title="کارگاه عملی UI/UX (تکمیل شده)"
                                            instructor="سارا احمدی"
                                            type="in-person"
                                            status="completed"
                                            schedule="پایان یافته"
                                            location="تهران، مرکز نوآوری"
                                            image="https://placehold.co/600x400/db2777/FFF?text=UI/UX"
                                        />
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="animate-in fade-in slide-in-from-bottom-8 duration-500">
                                <div className="bg-white dark:bg-slate-900 p-8 md:p-12 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-sm relative overflow-hidden">
                                    ...
                                    <div className="flex items-center gap-5 mb-10 pb-8 border-b border-slate-100 dark:border-slate-800">
                                        <div className="w-16 h-16 rounded-3xl bg-primary/10 dark:bg-slate-800 flex items-center justify-center text-primary shadow-inner">
                                            <Edit2 size={28} />
                                        </div>
                                        <div>
                                            <h3 className="text-2xl font-black text-slate-800 dark:text-white">ویرایش اطلاعات</h3>
                                            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">اطلاعات شخصی و امنیتی خود را بروزرسانی کنید</p>
                                        </div>
                                    </div>
                                    ...
                                    <form onSubmit={handleUpdateProfile} className="space-y-8">
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                            <div className="space-y-3">
                                                <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider flex items-center gap-1"><User size={14}/> نام و نام خانوادگی</label>
                                                <div className="relative group">
                                                    <input name="name" value={formData.name} onChange={handleChange} className="w-full px-5 py-4 bg-slate-50 dark:bg-slate-950 rounded-2xl border-2 border-slate-100 dark:border-slate-800 focus:border-primary dark:focus:border-primary outline-none transition-all dark:text-white font-bold text-sm focus:shadow-xl focus:shadow-primary/10" />
                                                </div>
                                            </div>
                                            <div className="space-y-3">
                                                <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider flex items-center gap-1"><Phone size={14}/> شماره موبایل</label>
                                                <div className="relative group">
                                        ...
                                        <div className="space-y-3">
                                            <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider flex items-center gap-1"><Mail size={14}/> ایمیل (غیرقابل تغییر)</label>
                                            <div className="relative opacity-60">
                                                <input value={formData.email} readOnly className="w-full px-5 py-4 bg-slate-100 dark:bg-slate-800/50 rounded-2xl border-2 border-slate-100 dark:border-slate-800 outline-none cursor-not-allowed dark:text-slate-400 font-bold dir-ltr text-left text-sm" />
                                            </div>
                                        </div>

                                        <div className="pt-8 border-t border-slate-100 dark:border-slate-800">
                                            <h4 className="text-base font-bold text-slate-800 dark:text-white mb-6 flex items-center gap-2">
                                                <span className="p-2 bg-amber-50 dark:bg-amber-900/20 text-amber-500 rounded-lg"><Lock size={18}/></span> تغییر رمز عبور
                                            </h4>
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                                <div className="relative group"><input type="password" name="newPassword" placeholder="رمز عبور جدید" className="w-full px-5 py-4 bg-slate-50 dark:bg-slate-950 rounded-2xl border-2 border-slate-100 dark:border-slate-800 focus:border-primary outline-none transition-all dark:text-white text-sm font-bold" /></div>
                                                <div className="relative group"><input type="password" name="confirmPassword" placeholder="تکرار رمز عبور جدید" className="w-full px-5 py-4 bg-slate-50 dark:bg-slate-950 rounded-2xl border-2 border-slate-100 dark:border-slate-800 focus:border-primary outline-none transition-all dark:text-white text-sm font-bold" /></div>
                                            </div>
                                        </div>

                                        <div className="flex justify-end pt-6">
                                            <Button type="submit" disabled={loading} className="!px-10 !py-4 !rounded-2xl !text-base shadow-2xl shadow-primary/30 hover:shadow-primary/50 hover:-translate-y-1 transition-all">
